...
物理相态与表征(可选·v1.8.3 起·法术AI 送审通过时才有) / 审核存档{原始描述,规范化结果} /
...
> 落库位置：回路记录顶层字段 `物理相态与表征`（可选，旧档/开局预设/免审回路没有该字段），
...
- 法术AI 调用失败/超时/未启用：bundle 侧降级引擎 `localJudge` 本地兜底（**兜底不产出「物理相态与表征」，该字段留空**）；双桌 UI 侧 15 秒超时令牌丢弃迟到响应，**通过/驳回/超时/错误都清送审槽、牌自动回牌库**。
