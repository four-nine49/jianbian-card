...
> ⚠️ 引擎已于 v1.8.0 升至 v2.2、**v1.8.5 升至 v2.3**：第一部分内嵌的引擎代码是 v2.1 快照——现行以 `src/gradband/engine/circuit-engine.js` 为准（新增：瞬时爆发线=持续×爆发倍率、地震引信口径 TUNE.seismicWorkFrac=2% 且引信类恒比爆发线、每轮预掷 本轮走火、亲和磨炼=全陌 ×4→×2 按 workE 实际做功能量计且生机系能量门控恒 ×1、**`quote()` 可选入参 req.固定 → 固化回路精神 ×0.2（÷5）**）。
...
- **亲和门控**：`TREE/FAMKEY/tierOf/gateOf`（主×1/次×2/陌×4）。
  ⚠️ **ctx 键名契约**：`tierOf` 读 `aff.main[].fam / .br`，取值是**中文族名**（内部 `FAMKEY[famKey]` 英文→中文再比），
  而存档 `亲和.主分支` 存的是 `{族, 分支}` —— 任何把存档亲和直接塞进 ctx 的地方都必须先转名
  （`settle.ts` 的 `affinityEngine()` 是官方范式；`opening.html` 的 `ctxRef()`、两个状态栏的 `affinityOf()` 各自内联一份）。
  漏转**不报错**，只会静默退化成「全陌」→ 精神×4/能量×4（v1.8.5 修过一次，别再犯）。
- **固定回路精神折扣**：`quote()` 的可选入参 `req.固定 = true`（宿主按 `type='fixed'` 传）→ 精神 ×0.2（÷5），
  与 `findTuned` 的 ×0.3 不叠加；自由/现构回路不传该位。
...
物理相态与表征(可选·v1.8.3 起；法术AI 送审通过写入，开局预设固定回路自 v1.8.4 起亦有) / 审核存档{原始描述,规范化结果} /
uses(free 次数, fixed=null) / 审核状态(免审|待送审|已通过) / 过载率(数字·键名不带百分号，旧文档误写「过载率%」是幽灵键) /
过载风险(0-100) / 本轮走火(boolean·脚本每轮结算预掷·判定权归脚本，世界书/叙事只可消费不可改判) / 桌面位置{x,y}
...
3. **回路认知**：列出固定/自由槽内回路（名/系·分支/一句效果/量级锚点/**形态锚点 `物理相态与表征`（外观唯一依据，
   严禁自行发挥光晕·魔法阵）**），告诉正文AI「只能且应该用这些」；**走火结论必须消费脚本预掷的 `本轮走火`**，
   条目里不要再自己掷骰（否则会与结算记录打架）。
...
> **你只需要按本文档重写 `提示词.数据AI` 与 `提示词.法术AI自创` / `提示词.法术AI剧情` 的文本**——
> 字段、占位符、枚举值一个都不能改，越权字段会被脚本整包打回。附录 A 是现有提示词（基线，写得不好的部分是你重点重写的对象）。
> （v1.8.7 起法术AI 拆成两套提示词：**自创申报** 与 **剧情获得送审** 各一套，AI 不再自行识别场景；
> 旧合并键 `提示词.法术AI` 仅作旧档迁移回退，不再有默认值。）
...
**v1.8.7 起拆成两套提示词、按调用场景各用各的**（AI 不再自行识别场景），输出形状有两套
（差异在 `规范化回路` 是否含 `参数向量`），写提示词时要对准自己那套：

| 调用场景 | 用的提示词 | 谁在调 | `描述` 是什么 | `参数` 是什么 | schema 名 | `规范化回路` 形状 |
|---|---|---|---|---|---|---|
| **A. 面板「申报入自由库」自创回路** | `提示词.法术AI自创` | 双桌 UI（出口 A） | 玩家在构筑台写的作用对象+作用方式原文（≥8 字） | `JSON{族, 参数明细(中文可读), 引擎参考效果}` | `spell_review` | `{名称, 一句话效果, 效果文字稿}`（**无参数向量**；参数由面板滑杆定） |
| **B. 剧情获得回路送审（送审槽/详情页「送审」）** | `提示词.法术AI剧情` | 双桌 UI（reviewCircuit） | 回路的「一句话效果」 | `参数模板JSON(famKey)`：顶层参数 ctrls + flow 子实体参数 + 默认参数 | `spell_review` | `{名称, 一句话效果, 参数向量}`（**必须含参数向量**，缺省键脚本用默认补齐） |
| C.（bundle 侧同构副本，当前无人调用） | spell-ai.ts（同上两套） | — | 同 A / 同 B | 同 A / 同 B | `spell_review` / `grant_review` | 同 A / 同 B |

> ⚠️ 两套提示词的输出要求与 json_schema 严格对齐：**自创套写「效果文字稿」、绝不提「参数向量」；
> 剧情套写「参数向量」、不要求「效果文字稿」**——schema 带 `additionalProperties:false`，
> 提示词要求了 schema 里没有的字段，AI 输出会被整包打回。
...
{ "结论": "通过",
  "规范化回路": { "名称": "风刃切", "一句话效果": "借风塑形高周波风刃",
                   "物理相态与表征": "沿手臂甩出收窄的透明波阵面，空气被压出可见的激波锥，伴随短促爆鸣，无光无焰",
                   "效果文字稿": "…施展描述…" },
  "解释": null }
// 或驳回：
{ "结论": "驳回", "规范化回路": null, "解释": "真空环境没有介质，声波无法传播" }

// B. 剧情获得：通过时必须带 参数向量（键值与引擎参数模板对应）+ 物理相态与表征
{ "结论": "通过",
  "规范化回路": { "名称": "风刃切", "一句话效果": "借风塑形高周波风刃",
                   "物理相态与表征": "收窄的透明激波锥，依赖现场空气，声学爆鸣",
                   "参数向量": { "mode": 1, "sub": 1, "fmed": 0, "fa": 30, "fv": 58, "br": 90 } },
  "解释": null }
```

> `物理相态与表征` **不是可选装饰**：它是正文AI 叙事时的唯一形态锚点（v1.8.3 起），
> 按五系机理分流写视觉/声学/空间/介质特征，严禁「绚丽光晕/魔法阵/神秘能量」这类奇幻辞藻。
> 落库位置：回路记录顶层字段 `物理相态与表征`（可选）。写它的有两条路径：①**法术AI 送审通过时**由 AI 产出；
> ②**开局送的固定回路自 v1.8.4 起也已带**，数据源在开局面板自己的 `BRANCH_CIRCUIT[br].表征`（不经过法术AI）。
> v1.8.3 之前生成的旧档没有该字段 → 双桌详情页「形态」行显示「—」。
> 另有 `审核存档.规范化结果` 保留法术AI 的完整原文。
...
- **`物理相态与表征`（v1.8.3 起）：正文AI 叙事唯一形态锚点**，必须依五系机理**分流**写视觉/声学/空间/介质——
  倾泻=发散准直与直线射束；波动·媒介波=介质依赖与激波声效；波动·场=无形势能范围与空间扰动；
  结构=微观晶格相态与就地取材附着（**绝无弹道**）；生机=渗透给药与生理组织异变（**绝无弹道与光球**）；
  感知=神经感官映射与无声无形球域。**严禁「绚丽光晕 / 魔法阵 / 神秘能量」等奇幻辞藻**。
...
- 法术AI 调用失败/超时/未启用：bundle 侧降级引擎 `localJudge` 本地兜底（**兜底不产出「物理相态与表征」，该字段留空**）；双桌 UI 侧 15 秒超时令牌丢弃迟到响应，**通过/驳回/超时/错误都清送审槽、牌自动回牌库**。注意**开局送的固定回路不受此降级影响**——它们的形态直接来自 `assets/opening.html` 的 `BRANCH_CIRCUIT`，根本不经过法术AI。
...
酒馆正文 AI 回复
  ...
  │
  └─ ✨ 法术AI（两种入口，v1.8.7 起各用各的提示词：A=法术AI自创 / B=法术AI剧情）
...
- 存放：`extensionSettings['渐变带自由回路'].提示词.{数据AI, 法术AI自创, 法术AI剧情}`（**已无感情AI**——v1.7.0 起删除，
  陆安追踪表改走标准填表，键位残留在 `gradband/core/settings.ts` 里也已清空不读取；
  **v1.8.7 起法术AI 拆成自创/剧情两套**，旧合并键 `法术AI` 只在读取时作迁移回退——新键缺失时沿用旧键内容
  （保住用户自定义），设置页保存一次后旧键消失）；
...
### A.2 法术AI·自创申报（`提示词.法术AI自创`，4 段 —— v1.8.7 自双场景版拆出）

```
[1 system 核心定位、自创申报契约与裁决铁律]
...
[2 system 法定五系18分支谱系与物理表征准则]
【世界观法定五系与 18 亲和分支物理运作与表征规范】
...
### A.3 法术AI·剧情获得送审（`提示词.法术AI剧情`，4 段 —— v1.8.7 自双场景版拆出）

```
[1 system 核心定位、剧情获得送审契约与裁决铁律]
你是跑团系统「渐变带」的法术AI，负责审核新回路在世界观内的「物理可行性与机制自洽性」。
...
[2 system 法定五系18分支谱系与物理表征准则]
—— 与 A.2 第 [2] 段完全相同（两套法术AI 提示词共用同一份谱系段）。
...
> ⚠️ 注意：以上是**新默认**（`core/settings.ts`）。**已有存档的 `extensionSettings` 旧合并键 `法术AI`
> 会在新键缺失时作为迁移回退**（两个新位先沿用旧内容，保住用户自定义）——在面板「渐变带·提示词」页
> 分别编辑/恢复默认并保存后，新键落盘、旧键消失，两套才真正分道扬镳。
...
*文档完。按本文档第 1、2 节重写提示词（数据AI 一套 + 法术AI 自创/剧情两套），保持占位符与 schema 不变，改善任务描写、对准各自输出形状、加固铁律。*
