...
(function (root, factory) {
  ...
  const TUNE = {
    // regenE_kJ_h / regenMind_h 已废弃：⑥c 自动恢复改按上限连续充能（能量 满回小时=max(6,72-48/9800×(Emax-200))，每分钟=Emax/(T×60)；精神 每分钟=Mmax/900），旧固定值不再使用
  ...
  /* ================= 编译：主角拥有的回路文本（喂正文AI的唯一形态） ================= */
  function compileOrder(q) {
    const r = q.r, L = [];
    L.push('【主角拥有的回路｜' + (q.tunedHit ? '固定招微调(' + q.tunedName + ')' : '自由回路·临时构建') + '】');
