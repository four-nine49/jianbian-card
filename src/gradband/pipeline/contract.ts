...
/** 剧情数值变更允许的键（白名单，其余=越权拒绝） */
export const 数值变更白名单 = ['能量', '精神', '能量上限', '精神上限'];
...
export function 校验变更包(raw: any, g: 游戏): 契约结果 {
  ...
  if (raw == null || typeof raw !== 'object') return { ok: false, error: '变更包不是 JSON 对象', 忽略的回路 };

  // 顶层字段白名单（§3.1-⑤ 越权字段整包打回）
  const allowed = ['现在剧情时间', '本轮使用回路', '剧情数值变更', '身体', '战斗中', '新增补给', '场景变更'];
  const extra = Object.keys(raw).filter(k => !allowed.includes(k));
  if (extra.length) return { ok: false, error: `越权字段：${extra.join('、')}（整包打回）`, 忽略的回路 };

  // 剧情数值变更键白名单
  const numKeys = Object.keys(raw.剧情数值变更 ?? {});
  const badNum = numKeys.filter(k => !数值变更白名单.includes(k));
  if (badNum.length) return { ok: false, error: `剧情数值变更含越权键：${badNum.join('、')}（施法费/物品/参数/亲和不得出现在这里）`, 忽略的回路 };

  // 身体只允许 {状态}
  if (raw.身体 != null) {
    const bk = Object.keys(raw.身体);
    const badBody = bk.filter(k => k !== '状态');
    if (badBody.length) return { ok: false, error: `身体字段含越权键：${badBody.join('、')}`, 忽略的回路 };
  }

  const parsed = 变更包Schema.safeParse(raw);
  if (!parsed.success) {
    const issue = parsed.error.issues[0];
    return { ok: false, error: `字段校验失败：${issue?.path?.join('.')} ${issue?.message}`, 忽略的回路 };
  }
  const data = parsed.data;

  // 时间解析 + 倒退检查
  const nowMin = 解析剧情时间(data.现在剧情时间);
  if (nowMin == null) return { ok: false, error: `剧情时间无法解析："${data.现在剧情时间}"（格式应为 2026年11月12日，21：12）`, 忽略的回路 };
  if (nowMin < g.主角.上次结算min) return { ok: false, error: `剧情时间倒退：${data.现在剧情时间} 早于上次结算（拒）`, 忽略的回路 };
